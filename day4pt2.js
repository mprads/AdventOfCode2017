const raw = `nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
const input = raw.split('\n')
    ...
    function findSum(arr) {
        return arr.reduce((sum, phrase) => {
            let valid = true;
            phrase.forEach((word1, word1Index) => {
                const letters1 = word1.split('').sort();
                phrase.forEach((word2, word2Index) => {
                    const letters2 = word2.split('').sort();
                    if ((word1 === word2 && word1Index !== word2Index) || (JSON.stringify(letters1) === JSON.stringify(letters2) && word1Index !== word2Index)) {
                        valid = false;
                    }
                });
            });
